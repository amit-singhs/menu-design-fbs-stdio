...
export default function EditMenuPage() {
  ...
  const { viewState, setMenuListView, setMenuDetailView, debugViewState } = useMenuViewStorage('edit-menu');
  ...
  useEffect(() => {
    ...
    if (viewState.view === 'menu-detail' && viewState.selectedMenuId && menus.length > 0) {
      ...
      console.log(`[Edit Menu] Menu exists check:`, { 
      ...
      if (!menuExists) {
        ...
        console.log(`[Edit Menu] Menu not found, resetting to menu list`);
        setMenuListView();
      } else {
        console.log(`[Edit Menu] Restoring menu:`, viewState.selectedMenuId);
  ...
  const handleMenuSelect = (menu: Menu) => {
    console.log(`[Edit Menu] Menu selected:`, menu.id, menu.name);
    setSelectedMenuId(menu.id);
    setMenuDetailView(menu.id);
  };

  const handleBackToMenus = () => {
    console.log(`[Edit Menu] Back to menus clicked`);
